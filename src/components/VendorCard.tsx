...
export function VendorCard({ vendor, onView, compact = false }: VendorCardProps) {
  ...
  const handleInstagram = (e: React.MouseEvent) => {
    ...
    if (vendor.social_media?.instagram) {
      const instagramUrl = vendor.social_media.instagram.startsWith('@') 
        ? `https://instagram.com/${vendor.social_media.instagram.substring(1)}`
              ...
              <div className="flex items-center gap-1">
                ...
                {vendor.social_media?.instagram && (
                  <button
                    ...
                    className="p-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded hover:from-purple-600 hover:to-pink-600 transition-colors"
